...
例：
ユーザー認証機能の詳細設計。design-alternatives.mdで評価した代替案1（JWT + Passport.js）を採用し、実装可能なレベルまで詳細化する。
...
**代替案の評価結果**
本設計は `design-alternatives.md` で評価した代替案<N>を採用しています。

**選定理由の要約**
<design-alternatives.mdでの選定理由を1-2段落で要約>

例：
代替案1（JWT + Passport.js）を採用。主な理由は以下の3点：
1. **保守性の高さ**: モジュール構成がシンプルで、チームメンバー全員が理解しやすい
2. **セキュリティ要件への適合**: OAuth2.0標準に準拠し、脆弱性リスクが最小化される
3. **既存システムとの親和性**: 現行の認証基盤と統合しやすく、移行コストが低い

**主要なトレードオフ**
<design-alternatives.mdで特定されたトレードオフを要約>
